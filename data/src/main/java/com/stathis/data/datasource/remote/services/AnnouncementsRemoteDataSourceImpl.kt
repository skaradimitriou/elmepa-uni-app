...
class AnnouncementsRemoteDataSourceImpl(
    ...
    override suspend fun fetchAnnouncementFromRemote(): Flow<NetworkResult<List<UiModel>>> =
        flow {
            try {
    ...
    override suspend fun fetchEventsFromRemote(): Flow<NetworkResult<List<UiModel>>> = flow {
        try {
